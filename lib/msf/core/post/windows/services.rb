...
#---# Native Meterpreter-specific windows service manipulation methods
	...
	def meterpreter_service_list()
		arr_of_hashes = meterpreter_get_service_hashes()
		ret_array = []
	...
	def meterpreter_service_list_running
		arr_of_hashes = meterpreter_get_service_hashes(0x01)
		ret_array = []
		arr_of_hashes.each do |h|
			##print_debug "h[:service_name]'s state is: #{h[:state].to_s}"
			if h[:state].to_s =~ /4/
				if @prefer_display_names
					ret_array << h[:display_name]
				else
	...
	def meterpreter_service_create(name, display_name, executable_on_host,mode=2)
		...
		serv_handle,scum_handle = get_serv_handle(0,nil,"SC_MANAGER_CREATE_SERVICE")
		begin
			...
			when 0 #success
				...
				handle_railgun_error(err,__method__,"Error creating service",
	...
	def meterpreter_service_start(service_name)
		...
		begin
			...
			if railhash["GetLastError"] == 0
				...
				err = railhash["GetLastError"]
				handle_railgun_error(err,__method__,"Error starting service",
	...
	def meterpreter_service_stop(service_name)
		...
		begin
			...
			if railhash["GetLastError"] == 0
				...
				err = railhash["GetLastError"]
				handle_railgun_error(err,__method__,"Error stopping service",
	...
	def meterpreter_service_delete(service_name)
		...
		begin
			...
			if railhash["GetLastError"] == 0
				...
				err = railhash["GetLastError"]
				handle_railgun_error(err,__method__,"Error deleting service",
	...
	def meterpreter_service_get_config(service_name)
		...
		if ! rg.advapi32.functions['QueryServiceConfigA']
			...
			#  		__out      LPDWORD pcbBytesNeeded
			#	);
			rg.add_function('advapi32', 'QueryServiceConfigA', 'BOOL', [
				['DWORD','hService',		'in'],
				['PBLOB','lpServiceConfig',	'out'],
				['DWORD','cbBufSize',		'in'],
		...
		begin
			...
			# passing in a buf size of 0 gives us the required buf size in pcbBytesNeeded
			if not railhash["GetLastError"] == 0 #change this to if == 0x7A (122) going forward?
				#then this is good, this puts buf size in pcBytesNeeded
				buf_size = railhash["pcbBytesNeeded"].to_i
			...
			#print_debug "Running QueryServiceConfigA with buf_size of #{buf_size}"
...
<<<<<<< HEAD
	def parse_query_service_config_structure(hex_string)
		...
		#  		LPTSTR lpDisplayName;
		#	} QUERY_SERVICE_CONFIG, *LPQUERY_SERVICE_CONFIG;
		_TYPES = [:DWORD,:DWORD,:DWORD,:LPSTR,:LPSTR,:DWORD,:LPSTR,:LPSTR,:LPSTR]
		_QUERY_SERVICE_CONFIG = SERVICE_CONFIG_STRUCT_NAMES.zip(_TYPES)
		#print_debug "parsing #{hex_string.inspect}\n using #{_QUERY_SERVICE_CONFIG.inspect}"
		h = session.railgun.util.read_struct(_QUERY_SERVICE_CONFIG,hex_string,0)
		...
		#print_debug "arr_of_strings is #{arr_of_strings.inspect}"
		arr_of_names = arr_of_strings.select {|item| item if item.length > 1}
		#print_debug "arr_of_names is #{arr_of_names.inspect}"
		hashish[:binary_path_name] = arr_of_names.shift
		hashish[:load_order_group] = arr_of_names.shift
